...
public class ServiceAspect {
    ...
    public Object aroundAdvice(ProceedingJoinPoint joinPoint){
        ...
        try{
            // 前置通知
            System.out.println("serviceAspect 环绕通知, try语句核心方法前, 目标方法是:"+ modifier+ " "+ name +" 使用的实参是:"+ Arrays.toString(args)+" 即将执行");
            // 目标核心方法在此执行
            result= joinPoint.proceed(args);
            // 返回通知
            System.out.println("serviceAspect 环绕通知, try语句核心方法后, 目标方法是:"+ modifier+ " "+ name +" 使用的实参是:"+ Arrays.toString(args)+" 执行结果是:"+ result);

        }catch (Throwable e){
            // 异常通知
            System.out.println("serviceAspect 环绕通知, catch语句, 目标方法是:"+ modifier+ " "+ name +" 使用的实参是:"+ Arrays.toString(args)+" 出现异常,异常信息是:"+ e.getMessage());
        }finally {
            // 最终通知
            System.out.println("serviceAspect 环绕通知, finally语句, 目标方法是:"+ modifier+ " "+ name +" 使用的实参是:"+ Arrays.toString(args)+" 执行结束");
        }
        return result;
